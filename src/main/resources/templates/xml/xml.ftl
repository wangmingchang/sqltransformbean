...
<mapper namespace="${daoPackageName}.${tableDto.daoName}">
    <resultMap id="BaseResultMap" type="${beanPackageName!''}.${tableDto.className}">
        <#list tableDto.primaryKeyColumnDtos as primaryKeyColumnDto>
            <id column="${primaryKeyColumnDto.columnName?substring(primaryKeyColumnDto.columnName?index_of('`')+1,primaryKeyColumnDto.columnName?last_index_of('`'))}"
                jdbcType="${primaryKeyColumnDto.columnType}" property="${primaryKeyColumnDto.fieldName}"/>
        </#list>
        <#list tableDto.columnDtos as columnDto>
            <#if !columnDto.ifKey>
                <result column="${columnDto.columnName?substring(columnDto.columnName?index_of('`')+1,columnDto.columnName?last_index_of('`'))}"
                        jdbcType="${columnDto.columnType}" property="${columnDto.fieldName}"/>
            </#if>
        </#list>
    </resultMap>
    ...
    <select id="selectByPrimaryKey"
            parameterType="<#if tableDto.primaryKeyColumnDtos?size gt 1>java.util.Map<#else>${tableDto.primaryKeyColumnDtos[0].fieldType}</#if>"
    ...
    <delete id="deleteByPrimaryKey"
            parameterType="<#if tableDto.primaryKeyColumnDtos?size gt 1>java.util.Map<#else>java.lang.String</#if>">
